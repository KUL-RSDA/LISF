...
!EOP
  ...
  call LIS_verify(status,&
       "ESMF_FieldGet for Soil Moisture Layer 1 failed in ac72_qcsoilm")

  call ESMF_AttributeGet(sm1Field,"Max Value",smmax1,rc=status)
  call LIS_verify(status,&
       "ESMF_AttributeGet: Max Value failed in ac72_qcsoilm")

  call ESMF_AttributeGet(sm1Field,"Min Value",smmin1,rc=status)
  call LIS_verify(status,&
       "ESMF_AttributeGet: Min Value failed in ac72_qcsoilm")
  ...
  call LIS_verify(status,&
       "ESMF_StateGet for Soil Moisture Layer 2 failed in ac72_qcsoilm")
 
  call ESMF_FieldGet(sm2Field,localDE=0,farrayPtr=soilm2,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet for Soil Moisture Layer 2 failed in ac72_qcsoilm")
  ...
  call LIS_verify(status,&
       "ESMF_StateGet for Soil Moisture Layer 3 failed in ac72_qcsoilm")
 
  call ESMF_FieldGet(sm3Field,localDE=0,farrayPtr=soilm3,rc=status)
  call LIS_verify(status,&
       "ESMF_FieldGet for Soil Moisture Layer 3 failed in ac72_qcsoilm")
  ...
  call LIS_verify(status, &
       "ESMF_StateGet for Soil Moisture Layer 4 failed in ac72_qcsoilm")

  call ESMF_FieldGet(sm4Field,localDE=0,farrayPtr=soilm4,rc=status)
  call LIS_verify(status, &
       "ESMF_FieldGet for Soil Moisture Layer 4 failed in ac72_qcsoilm")
  ...
  call LIS_verify(status, &
       "ESMF_StateGet for Soil Moisture Layer 5 failed in ac72_qcsoilm")

  call ESMF_FieldGet(sm5Field,localDE=0,farrayPtr=soilm5,rc=status)
  call LIS_verify(status, &
       "ESMF_FieldGet for Soil Moisture Layer 5 failed in ac72_qcsoilm")
  ...
  call LIS_verify(status, &
       "ESMF_StateGet for Soil Moisture Layer 6 failed in ac72_qcsoilm")

  call ESMF_FieldGet(sm6Field,localDE=0,farrayPtr=soilm6,rc=status)
  call LIS_verify(status, &
       "ESMF_FieldGet for Soil Moisture Layer 6 failed in ac72_qcsoilm")
  ...
  call LIS_verify(status, &
       "ESMF_StateGet for Soil Moisture Layer 7 failed in ac72_qcsoilm")

  call ESMF_FieldGet(sm7Field,localDE=0,farrayPtr=soilm7,rc=status)
  call LIS_verify(status, &
       "ESMF_FieldGet for Soil Moisture Layer 7 failed in ac72_qcsoilm")
  ...
  call LIS_verify(status, &
       "ESMF_StateGet for Soil Moisture Layer 8 failed in ac72_qcsoilm")

  call ESMF_FieldGet(sm8Field,localDE=0,farrayPtr=soilm8,rc=status)
  call LIS_verify(status, &
       "ESMF_FieldGet for Soil Moisture Layer 8 failed in ac72_qcsoilm")
  ...
  call LIS_verify(status, &
       "ESMF_StateGet for Soil Moisture Layer 9 failed in ac72_qcsoilm")

  call ESMF_FieldGet(sm9Field,localDE=0,farrayPtr=soilm9,rc=status)
  call LIS_verify(status, &
       "ESMF_FieldGet for Soil Moisture Layer 9 failed in ac72_qcsoilm")
  ...
  call LIS_verify(status, &
       "ESMF_StateGet for Soil Moisture Layer 10 failed in ac72_qcsoilm")

  call ESMF_FieldGet(sm10Field,localDE=0,farrayPtr=soilm10,rc=status)
  call LIS_verify(status, &
       "ESMF_FieldGet for Soil Moisture Layer 10 failed in ac72_qcsoilm")
  ...
  do t=1,LIS_rc%npatch(n,LIS_rc%lsm_index)
     if(soilm10(t).gt.smmax10) soilm10(t) = smmax10
